...
Same package id (`mimo-provider`), same publisher (`drdelco`) — existing installations upgrade in place.

### New: Multi-agent Orchestra

- **`Orchestra: Execute Complex Task`** command opens a planning/execution pipeline:
  1. **Architect** breaks the request into Work Orders (deliverables + acceptance criteria + dependencies)
  2. **Agents** execute Work Orders in parallel via DAG resolution
  3. **Security reviewer** audits all touched files (OWASP Top 10, secrets, injection)
  4. **Architect** synthesizes the final report

- **5 providers** in the orchestra: MiMo, Kimi, DeepSeek, MiniMax, Claude
  - Each provider has its own concurrency pool (Kimi: 300, DeepSeek: 50, MiniMax: 20, MiMo: 10, Claude: 5)
  - Auto-fallback: if a provider fails, the work order retries on the next preferred provider for that role

- **Inter-agent messaging** via Mailbox: agents can `ask_agent`, `notify`, `broadcast`, `check_inbox` mid-execution

- **Sidebar live view** (`mimo.orchestraView`): real-time activity feed with agent cards, message log, security review

- **TF-IDF semantic memory**: each agent receives semantically relevant context from prior Work Orders and messages

### Configuration

- New settings: `orchestra.budgetLimit`, `orchestra.poolLimits`, `orchestra.autoFallback`, `orchestra.skipSecurityReview`, `orchestra.director`
- Existing `mimo.*` settings (kimiApiKey, deepseekApiKey, minimaxApiKey, claudeApiKey) are reused for orchestra providers — no duplicate config

### Single-agent chat (preserved from 1.1.0)

All polished single-agent chat features remain: streaming, syntax highlighting, copy code button, model selector, export, web search, persistent tabs, OAuth login.
